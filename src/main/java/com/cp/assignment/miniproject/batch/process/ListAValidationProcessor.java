...
@Component
public class ListAValidationProcessor implements ItemProcessor<ListATransaction, ListATransaction> {
    ...
    private String validate(ListATransaction item) {
        // Errors that must go to Error_Records.csv
        if (item == null) {
            throw new ValidationException(
                    "List A transaction is null"
            );
        }

        // Errors that must go to Error_Records.csv
        if (CommonUtils.isBlank(item.getOrderNumber())) {
            throw new ValidationException(
